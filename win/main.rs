...
mod implementation {
    ...
    use winapi::{
        ...
        um::{
            consoleapi::{
                GetConsoleMode, ReadConsoleInputW, SetConsoleCtrlHandler, SetConsoleMode,
            },
            ...
            winbase::{
                ...
                GMEM_MOVEABLE, INFINITE, NORMAL_PRIORITY_CLASS, PIPE_ACCESS_DUPLEX,
            ...
            winuser::{
                ...
                CF_UNICODETEXT, VK_BACK, VK_DELETE, VK_DOWN, VK_END, VK_ESCAPE, VK_F1, VK_F24,
                VK_HOME, VK_LEFT, VK_NEXT, VK_PRIOR, VK_RETURN, VK_RIGHT, VK_SPACE, VK_TAB, VK_UP,
            },
        },
    };
    ...
    use pepper::{
        application::{
            AnyError, ApplicationEvent, ClientApplication, ProcessTag, ServerApplication,
        },
        client::ClientHandle,
        platform::{
            BufPool, ExclusiveBuf, Key, Platform, PlatformRequest, ProcessHandle, SharedBuf,
        },
        Args,
    };
    ...
    impl ConnectionToClientListener {
        fn new_listen_reader(pipe_path: &[u16], buf_len: usize) -> AsyncReader {
            ...
            if unsafe { ConnectNamedPipe(reader.handle().0, reader.overlapped().as_mut_ptr()) }
                != FALSE
            {
    ...
    fn run_server(args: Args, pipe_path: &[u16]) -> Result<(), AnyError> {
        ...
        let mut client_connections: [Option<ConnectionToClient>; MAX_CLIENT_COUNT] =
            Default::default();
    ...
    fn run_client(args: Args, pipe_path: &[u16], input_handle: HANDLE, output_handle: HANDLE) {
        ...
        let mut console_event_buf =
            [unsafe { std::mem::zeroed() }; CLIENT_CONSOLE_EVENT_BUFFER_LEN];
